...
class ResNet(nn.Module):
    def __init__(self, block, num_filters, num_blocks, num_classes=10):
        ...
        self.layer2 = self._make_layer(block, 128, num_blocks[1], num_filters, stride=2)
        self.layer3 = self._make_layer(block, 256, num_blocks[2], num_filters, stride=2)
        self.layer4 = self._make_layer(block, 512, num_blocks[3], num_filters, stride=2)
        self.linear = nn.Linear(512*block.expansion, num_classes)

    def _make_layer(self, block, planes, num_blocks, num_filters, stride):
        strides = [stride] + [1]*(num_blocks-1)

        layers = []
        for stride in strides:
            layers.append(block(self.in_planes, planes, num_filters, stride))
